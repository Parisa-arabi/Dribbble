  ...
  try {
    await session.withTransaction(async () => {
      ...
      const imageUrls = req.files.map(
        (file) => `/uploads/designs/${file.filename}`
      );
...
exports.viewDesigns = async (req, res) => {
  try {
    ...
    const designerObjectId =
      typeof designerId === "string" ? new ObjectId(designerId) : designerId;
    const designs = await designsCollection
      .find({ designer: designerObjectId })
      .toArray();
    ...
    const transformedDesigns = await Promise.all(
...
exports.getDesignerIncome = async (req, res) => {
  try {
    ...
    const designsWithPurchases = await Promise.all(
      designs.map(async (design) => {
        try {
          ...
          const formattedBuyers = buyers.map((buyer) => {
            const purchase = buyer.PurchasesList.find(
              (p) => p.DesignID === design._id.toString()
            );
    ...
    const totalIncome = designsWithPurchases.reduce(
...
exports.editDesign = async (req, res) => {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const design = await designsCollection.findOne(
        { _id: designId },
        { session }
      );
      if (!design) {
